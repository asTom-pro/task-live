...
debug('はい'.print_r($currentPageNum,true));
$currentMinNum = ($currentPageNum - 1) * 20 + 1;
// 部屋名検索
...
$sort = 'DESC';
$span = 20;
$dbRoomsInfo = getRoomsInfo($currentMinNum, $sort, $search, $span);
$totalPageNum = $dbRoomsInfo['total_page'];
...
if (!empty($_GET)) {
  ...
  if (!empty($_GET['tag'])) {
    ...
    $tag_id = getTagId($tag);
    if (empty($tag_id)) {
      debug('不正なパラメータ（タグ）を取得したのでトップページに遷移します。');
...
if (empty($dbRoomsInfo['total'])) {
  debug('不正なパラメータ（部屋）を取得したのでトップページに遷移します。');
  debug('取得した部屋情報' . print_r($dbRoomsInfo, true));
  header('Location:index.php');
  exit();
}

$no_room_flg = false;
if (count($dbRoomsInfo['data'][0]) == 0) {
  $no_room_flg = true;
}
$title = '部屋一覧';
?>
    ...
    <div class="container">
      <div class="main-bar">
        <div class="room-container">
                ...
                <div class="room">
                  <div class="room-info">
                    ...
                    <div class="room-user-info room-info-list">
                      ...
                      <object>
                        <a href="userpage.php?u=<?php if (!empty($roomUserId) && $roomUserId != 1) {
                                                  echo sanitize($roomUserId);
                                                } ?>">
                          <img src="
                        <?php
                  ...
                  <div class="room-time">
                    ...
                    <div class="remain-time">
                      <span class="remain-time-num-m"><?php if (!empty($limit_time_min)) {
                                                        echo sanitize($limit_time_min);
                                                      } else {
                                                        echo '0';
                                                      } ?></span>/
                      <?php if (!empty($roomMaxTime)) {
          ...
          <?php } ?>
          <?php
          foreach ($dbRoomsInfo['data'] as $key => $val) {
          ...
          ?>
              <a href="room.php?r_id=<?php if (!empty($val['room_id'])) {
                                        echo sanitize($val['room_id']);
                                      } ?>" class="room-link">
                <div class="room">
                  <div class="room-info">
                    ...
                    <div class="room-member room-info-list">
                      <div class="member-container">
                        ...
                        <span class="member-num">
                          <?php
                          if ($isRoomTimeOut) {
                            if (!empty($endRoomUserNum)) {
                              echo sanitize($endRoomUserNum);
                            }
                          } else {
                            echo sanitize($nowRoomUserNum);
                          }
                          ?>
                        </span>人
                      </div>
                    ...
                    <div class="room-user-info room-info-list">
                      ...
                      ?>
                      <object>
                        <a href="userpage.php?u=<?php if (!empty($roomUser['id']) && $roomUser['id'] != 1) {
                                                  echo sanitize($roomUser['id']);
                                                } ?>">
                          <img src="<?php if (!empty($roomUser['profile_img'])) {
                                      echo sanitize($roomUser['profile_img']);
                                    } else {
                                      echo 'img/user-sample.svg';
                                    } ?>" alt="" class="user-img">
                          <span class="user-name">
        ...
        <?php pagenation($totalPageNum, $currentPageNum); ?>
        </div>
      </div>
      <div class="sub-bar">
            ...
            <label for="">
              ...
              <select name="tag" class="select">
                ...
                <?php foreach ($dbTags as $key => $val) { ?>
                  <option value="<?php echo sanitize($val['room_tag_name']); ?>" <?php
                                                                                  if (!empty($tag)) {
